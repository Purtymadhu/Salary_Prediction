...
def graph(X_train,Y_train,regressionObject,X_test,Y_test,Y_pred):
    plt.scatter(X_train,Y_train,color='red',label='Madhu training data')
    plt.plot(X_train,regressionObject.predict(X_train),color='blue',label='Best Fit')
    plt.scatter(X_test,Y_test,color='green',label='test data')
    plt.scatter(X_test,Y_pred,color='black',marker='*',label='Madhu Pred test data')
...
def main():
    ...
    try:
        ...
        i=0
        print('X_test_data','...','Y_test_data','...','Y_pred_data')
        print("\n")
        while i<len(X_test):
        ...
        plt.scatter(experience,salaries,color='red',marker='*')
        plt.xlabel('Years of Experience')
        plt.ylabel('Salaries')
        plt.show()
